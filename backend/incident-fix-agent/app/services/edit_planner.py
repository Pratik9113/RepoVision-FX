...
def build_edit_plan(
    ...
    for cf in candidate_files:
        ...
        focus_lines = sorted({m.get("line") for m in matches if isinstance(m, dict) and m.get("line")})[:20]
    ...
    # Choose high-confidence files to auto-edit
    ...
    if high:
        ...
        # Keep only files that are close to the best one (within 10 points)
        near_top = [h for h in high if h["confidence"] >= max_conf - 10]

        # Prefer files that have direct evidence: explicit file path or function signals
        ...
        for h in near_top:
            ...
            has_func = any(
                fm.get("file") == h["file"] and fm.get("function_name") in func_signals
                for fm in (function_matches or [])
            )
            if has_fp or has_func:
                strong.append(h)

        candidates = strong or near_top
        selected = candidates[:MAX_FILES_TO_EDIT]

    reason: str
    if not selected:
        # No truly high-confidence file – be conservative and skip auto-edits
        top = max(annotated, key=lambda x: x["confidence"])
        reason = (
