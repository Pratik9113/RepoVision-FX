...
import type { ThreeDGraph, GraphNode, GraphLink } from '../types';
...
export const ThreeDGraphViewer: React.FC<ThreeDGraphProps> = ({ data }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const rendererRef = useRef<THREE.WebGLRenderer | null>(null);
  const cameraRef = useRef<THREE.PerspectiveCamera | null>(null);
  const sceneRef = useRef<THREE.Scene | null>(null);
  const [focus, setFocus] = useState<'all' | 'frontend' | 'backend'>('all');
  ...
  useEffect(() => {
    ...
    const createDecoratedGlobe = (color: number, title: string) => {
      ...
      (g as any).isGlobeTrigger = true;
      (g as any).layerType = title.toLowerCase();
    ...
    const nodesLabels = new Map<string, THREE.Mesh>();
    const arrowsToAnimate: { arrows: THREE.Group[], curve: THREE.CatmullRomCurve3 }[] = [];

    // Help determine file color based on type
    const getFileColor = (node: GraphNode) => {
    ...
    const placeNodesOnGlobe = (layerNodes: GraphNode[], group: THREE.Group) => {
    ...
    const onMouseDown = (e: MouseEvent) => {
      ...
      for (const intersect of intersects) {
        let parent: any = intersect.object;
    ...
    const animate = () => {
      ...
      groupRotation.setFromEuler(new THREE.Euler(rotation.x, rotation.y, 0));
